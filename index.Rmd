...
###### NEED TO ADD THE PACKAGES THAT WE USE AND A SHORT DESCRIPTION #########
...
```
From our anaylsis above you can see that we obtained a pvalue lower than 0.05. Hence we rejected our initial hypothesis on the affects of room type on the price of the airbnb. 
```{r}
  test <- abnb %>%
    slice(20:25)

  auto_fit <- aov(price~neighbourhood, data=test)
  summary(auto_fit)
  TukeyHSD(auto_fit)

```
From our analysis we can see that we can not reject our initial hypothesis of whether there is a relationship between price and neighbourhoods. The pvalue we obtained in the analysis above is greater than 0.05 and therefore we shall not reject our initial hypothesis.


# Part 4 Macnhine Learning
```{r}
...
```
We are going to reject the null hypothesis of no relationship between the time of year and price. As our pvalue shows there is a significant relationship between price and the time of year. As the years go by there is a slight decrease in the price of airbnbs. 
